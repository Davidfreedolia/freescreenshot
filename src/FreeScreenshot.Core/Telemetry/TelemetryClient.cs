...
namespace FreeScreenshot.Core.Telemetry;

/// <summary>
/// Thin client over the freedolia.com/api/freescreenshot/* endpoints.
...
{
    private const string BaseUrl = "https://freedolia.com/api/freescreenshot";

    // Public anti-spam key. Not a secret (it ships inside the desktop binary)
    // — just enough to keep random callers off the endpoint.
    private const string PublicKey = "freescreenshot-public-v1";
    ...
    public TelemetryClient(AppConfig config, HttpClient? http = null)
    {
        ...
        if (!_http.DefaultRequestHeaders.Contains("X-Freescreenshot-Key"))
        {
            _http.DefaultRequestHeaders.Add("X-Freescreenshot-Key", PublicKey);
        }
    }
